...
std::unique_ptr<MappedFile> MappedFile::FromFd(borrowed_fd fd, off64_t offset, size_t length, int prot) {
#if defined(_WIN32)
...
std::unique_ptr<MappedFile> MappedFile::FromOsHandle(os_handle h, off64_t offset, size_t length, int prot) {
...
#if defined(_WIN32)
    HANDLE handle = CreateFileMappingW(h, nullptr, (prot & PROT_WRITE) ? PAGE_READWRITE : PAGE_READONLY, 0, 0, nullptr);
    if (handle == nullptr) {
        // http://b/119818070 "app crashes when reading asset of zero length".
        // Return a MappedFile that's only valid for reading the size.
        if (length == 0 && ::GetLastError() == ERROR_FILE_INVALID) {
            return std::unique_ptr<MappedFile>(new MappedFile(const_cast<char *>(kEmptyBuffer), 0, 0, nullptr));
        }
        return nullptr;
    }
    void *base = MapViewOfFile(handle, (prot & PROT_WRITE) ? FILE_MAP_ALL_ACCESS : FILE_MAP_READ, (file_offset >> 32),
                               (DWORD)file_offset, file_length);
    if (base == nullptr) {
        CloseHandle(handle);
        return nullptr;
    }
    return std::unique_ptr<MappedFile>(new MappedFile(static_cast<char *>(base), length, slop, handle));
#else
